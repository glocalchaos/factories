...
class Parser:
    def __init__(self, 
                 workbook_path='/home/GFKAA3/Projects/factories/src/data.xlsx'):
                #  placeholder_factory_name='НЕИЗВЕСТЕН'):
    ...
    def parse_products_categories(self) -> Dict[str, str]:
        ...
        for row in service_sheet.iter_rows(min_row=2, max_row=38,
                                           min_col=2, max_col=3):  # &TODO fix hardcode
            product_name = row[0].value.strip()
            category_name = row[1].value.strip()
            # TODO когда парсим, надо потом сохранить наверное айдишники
            # TODO категорий в кеше, чтобы не искать из потом в бд
